    ...
    public class FTDI_Manager: IDisposable
    {
        ...
        public FTDI.FT_DEVICE_INFO_NODE[] DeviceList
        {
            get
            {
                ...
                var device = new FTDI.FT_DEVICE_INFO_NODE();
                device.Description = "Fake Description";
                device.SerialNumber = "Fake Serial Number";
                device.ID = 5;
                device.LocId = 2;
                return new[] { device };
            }
        ...
        public uint Write(byte[] bytesForWriting)
        {
            ...
            var k =  (3000001 - _baudRate)/(double) 2884800;
            var timeout = (int)(bytesForWriting.Length*k);
            MicroStopwatch.USleep(timeout);
            return (uint)bytesForWriting.Length;
        }

        private uint countLoop = 0;
        ...
        public byte[] Read(uint expectedNumberOfBytes = 1, uint timeoutMs = 2000, bool isDoEvents = true)
        {
            ...
            var k = (3000001 - _baudRate) / (double)2884800;
            var timeout = (int)(500 * k);
            MicroStopwatch.USleep(timeout);
            return new Packet(0, 0x1).ToBytes();
            if (countLoop > 9500)
            {
                countLoop = 0;
                return new Packet(0, 0x1).ToBytes();
            ...
            ++countLoop;
            return new Packet(6, 0x0).ToBytes();
        ...
        private void ReadDataHandler_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                ...
                while (!ReadDataHandler.CancellationPending)
                {
                    ...
                    {
                        OnRecvBytes(new Packet(6, new List<ushort> {count++}).ToBytes());
